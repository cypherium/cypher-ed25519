...
import (
	...
	//	"github.com/cypherium/cypherBFT/consensus/clique"
...
func (api *SignerAPI) sign(addr common.MixedcaseAddress, req *SignDataRequest, legacyV bool) (hexutil.Bytes, error) {
	...
	signature, err := wallet.SignHashWithPassphrase(account, pw, signHash(req.Rawdata))
...
func cliqueHeaderHashAndRlp(header *types.Header) (hash, rlp []byte, err error) {
	err = fmt.Errorf("clique header but no cliqueHeaderHashAndRlp")
	return
	/*
		if len(header.Extra) < 65 {
