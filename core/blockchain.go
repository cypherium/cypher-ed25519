...
import (
	...
	"github.com/cypherium/cypherBFT/core/state/snapshot"
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64, kbc *KeyBlockChain) (*BlockChain, error) {
	...
	var err error
	bc.hc, err = NewHeaderChain(db, chainConfig, bc.Validator, bc.insertStopped)
	if err != nil {
		return nil, err
	}
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db ethdb.KeyValueWriter, header *types.Header) {
		...
		if currentBlock := bc.CurrentBlock(); currentBlock != nil && header.Number.Uint64() < currentBlock.NumberU64() {
			...
			atomic.StoreUint64(&bc.currentBlockN, newHeadBlock.NumberU64())
		}
...
func (bc *BlockChain) ResetWithGenesisBlock(genesis *types.Block) error {
	...
	atomic.StoreUint64(&bc.currentBlockN, bc.genesisBlock.NumberU64())
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	if ptd == nil {
		return NonStatTy, types.ErrUnknownAncestor
	}
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, error) {
	...
	case errors.Is(err, types.ErrPrunedAncestor):
	...
	case errors.Is(err, types.ErrFutureBlock) || (errors.Is(err, types.ErrUnknownAncestor) && bc.futureBlocks.Contains(it.first().ParentHash())):
		for block != nil && (it.index == 0 || errors.Is(err, types.ErrUnknownAncestor)) {
	...
	if block != nil && errors.Is(err, types.ErrFutureBlock) {
		if err := bc.addFutureBlock(block); err != nil {
			return it.index, err
		}
		block, err = it.next()

		for ; block != nil && errors.Is(err, types.ErrUnknownAncestor); block, err = it.next() {
			if err := bc.addFutureBlock(block); err != nil {
				return it.index, err
			}
			stats.queued++
